...
async def payload(address):
    now = datetime.now()
    msg = "Tezos Signed Message: Confirming my identity as {address} on https://dsr-team.github.io/, time: {timestamp}".format(address=address, timestamp=now)
...
async def login(address: str = Form(""), signature: str = Form("") ):
    pubKey = requests.get('https://api.tzkt.io/v1/accounts/{address}'.format(address=address)).json()['publicKey']
    
    cursor = Col_login.find({'address':address}).sort('time',pymongo.DESCENDING).limit(1)
...
async def create_room(name: str = Form(""), image: str = Form(""), address=Depends(get_current_active_user)):
    ...
    while True:
        ...
        if len(list(cursor))==0:
            break
    Col_room.insert_one({'name':name, 'image':image, 'owner':address, "id":id, "created_at":datetime.now(), "updated_at":datetime.now(), "metadata":[{},{},{},{},{},{}]})
    return {"id":id}
...
async def update_room(update_dict:UpdateRoomModel, room_id="", address=Depends(get_current_active_user)): #
    ...
    print(update_dict)
    try:
        result = Col_room.update_many({'id': room_id, "owner":address}, {"$set": update_dict})
...
async def get_token_metadata(contract:str, tokenId: int):   
    result = requests.get("https://api.akaswap.com/v2/fa2tokens/{contract}/{tokenId}".format(contract=contract, tokenId=tokenId)).json()
...
async def get_collections_by_address(mimeTypes: str|None="", offset:int | None=0, address=""):#offset:int | None=0, mimeTypes:str | None="", address=""):
